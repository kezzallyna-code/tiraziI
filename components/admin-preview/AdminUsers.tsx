...
export function AdminUsers({ triggerToast }: { triggerToast: (msg: string) => void }) {
  ...
  return (
    <div className="bg-white dark:bg-surface-container-low p-6 rounded-[24px] border border-outline-variant/10 shadow-sm space-y-6 animate-in fade-in duration-300">
      <div>
        <h3 className="font-headline-sm text-headline-sm text-primary font-serif">Platform Users</h3>
        <p className="font-body-md text-on-surface-variant mt-1">Manage and moderate platform accounts.</p>
      </div>
              ...
              <tr key={u.id} className="hover:bg-surface-container/10">
                ...
                <td className="py-4 text-right pr-2 space-x-2">
                  <button 
                    ...
                    className="px-3 py-1 bg-transparent hover:bg-surface-container border border-outline-variant rounded-lg text-[12px] font-bold text-on-surface transition-colors"
                  >
                    View
                  </button>
                  <button 
                    onClick={() => triggerToast("Admin action preview only.")}
                    className="px-3 py-1 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg text-[12px] font-bold text-red-700 transition-colors"
                  >
                    Suspend
                  </button>
                </td>
