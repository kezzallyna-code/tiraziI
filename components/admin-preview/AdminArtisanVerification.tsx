...
export function AdminArtisanVerification({ triggerToast }: { triggerToast: (msg: string) => void }) {
  ...
  const handleAction = (id: string, action: 'Approved' | 'Rejected') => {
    setRequests(prev => prev.map(req => {
      if (req.id === id) {
        triggerToast(`Verification request ${action.toLowerCase()} (preview only).`);
  ...
  return (
    <div className="bg-white dark:bg-surface-container-low p-6 rounded-[24px] border border-outline-variant/10 shadow-sm space-y-6 animate-in fade-in duration-300">
      <div>
        <h3 className="font-headline-sm text-headline-sm text-primary font-serif">Artisan Verification</h3>
