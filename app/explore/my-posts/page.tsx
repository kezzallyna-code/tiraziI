...
export default function MyPostsPage() {
  const { posts, isInitialized, currentUser } = useExplore();

  // Filter posts where author is the current user
  const myPosts = currentUser ? posts.filter((post) => post.artisan.id === currentUser.id) : [];
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white dark:bg-surface-container-low p-6 rounded-[24px] border border-outline-variant/30 shadow-sm">
        <h2 className="font-headline-md text-headline-md text-primary mb-1">My Posts</h2>
        <p className="font-body-md text-on-surface-variant">Your atelier updates, thoughts, and shared creations.</p>
      </div>
      ...
      {myPosts.length > 0 ? (
        ...
        <div className="bg-white dark:bg-surface-container-low p-12 rounded-[24px] border border-outline-variant/20 shadow-sm text-center">
          ...
          <h3 className="font-headline-sm text-[20px] text-on-surface mb-2">You have not shared anything yet.</h3>
          <p className="font-body-md text-on-surface-variant max-w-sm mx-auto leading-relaxed mb-6">
            Share your first textile idea, tip, question, or creation with the community.
          </p>
          <Link
            href="/explore"
